import {
  DEFAULT_MEMBERSHIP_BALANCE_THRESHOLD,
  normalizeMembershipBalanceThreshold,
} from './membership'

export const DEFAULT_FIRST_DEPOSIT_BONUS_PERCENT = 3
export const DEFAULT_MIN_WITHDRAWAL_AMOUNT = 6000
export const WITHDRAWAL_HARD_LIMIT_AMOUNT = 60000
export const DEFAULT_DAILY_WITHDRAWAL_LIMIT = 60000
export const DEFAULT_ANNUAL_WITHDRAWAL_LIMIT = 60000000
// Kept as an alias for older callers. This value is an annual total, not a
// per-request limit; every request is capped by WITHDRAWAL_HARD_LIMIT_AMOUNT.
export const DEFAULT_MAX_WITHDRAWAL_AMOUNT = DEFAULT_ANNUAL_WITHDRAWAL_LIMIT
export const DEFAULT_WITHDRAWAL_FEE_PERCENT = 7
export const DEFAULT_WITHDRAWALS_ENABLED = true
export const DEFAULT_WITHDRAWAL_DISABLED_MESSAGE = 'Withdrawals are temporarily unavailable. Please try again later.'
export const DEFAULT_TELEGRAM_GROUP_URL = 'https://t.me/+Giav1o1JVGNkYzNk'
export const DEFAULT_WHATSAPP_GROUP_URL = 'https://chat.whatsapp.com/I1D6NNWndu6HDrbzB5BkPX?s=hd&p=i&mlu=0&ilr=0'
export const DEFAULT_CUSTOMER_SUPPORT_URL = 'https://t.me/EFC_Support'

function normalizeBonusPercent(value) {
  const percent = Number(value)
  if (!Number.isFinite(percent) || percent < 0 || percent > 100) {
    const error = new Error('First deposit bonus percent must be between 0 and 100')
    error.statusCode = 400
    throw error
  }

  return Number(percent.toFixed(3))
}

function normalizeWithdrawalAmount(value, fieldName) {
  const amount = Number(value)
  if (!Number.isFinite(amount) || amount < 0) {
    const error = new Error(`${fieldName} must be zero or greater`)
    error.statusCode = 400
    throw error
  }

  return Number(amount.toFixed(3))
}

function normalizeWithdrawalFeePercent(value) {
  const percent = Number(value)
  if (!Number.isFinite(percent) || percent < 0 || percent > 100) {
    const error = new Error('Withdrawal fee percent must be between 0 and 100')
    error.statusCode = 400
    throw error
  }

  return Number(percent.toFixed(3))
}

function normalizeExemptUsernames(value) {
  const values = Array.isArray(value) ? value : String(value || '').split(/[\n,]/)
  return [...new Set(values.map((username) => String(username).trim().toLowerCase()).filter(Boolean))]
}

function normalizeWithdrawalDisabledMessage(value) {
  const message = String(value ?? '').trim()
  if (message.length > 1000) {
    const error = new Error('Withdrawal disabled message cannot exceed 1000 characters')
    error.statusCode = 400
    throw error
  }

  return message || DEFAULT_WITHDRAWAL_DISABLED_MESSAGE
}

function normalizeExternalUrl(value, fieldName, fallback) {
  const url = String(value ?? '').trim() || fallback
  if (url.length > 2000) {
    const error = new Error(`${fieldName} cannot exceed 2000 characters`)
    error.statusCode = 400
    throw error
  }

  try {
    const parsed = new URL(url)
    if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('Unsupported protocol')
    return parsed.toString()
  } catch {
    const error = new Error(`${fieldName} must be a valid http or https URL`)
    error.statusCode = 400
    throw error
  }
}


function isMissingSettingsTable(error) {
  return error?.code === '42P01' || /admin_settings/i.test(error?.message || '')
}

function isMissingSettingsColumn(error, column) {
  const message = `${error?.message || ''} ${error?.details || ''} ${error?.hint || ''}`.toLowerCase()
  return error?.code === '42703' || message.includes(`column ${column.toLowerCase()}`) || message.includes(column.toLowerCase()) && message.includes('schema cache')
}

export async function getMembershipBalanceThreshold(supabase, { allowDefaultOnMissingTable = true } = {}) {
  const { data, error } = await supabase
    .from('admin_settings')
    .select('membership_balance_threshold')
    .eq('id', 1)
    .maybeSingle()

  if (error) {
    if (allowDefaultOnMissingTable && (isMissingSettingsTable(error) || isMissingSettingsColumn(error, 'membership_balance_threshold'))) {
      return DEFAULT_MEMBERSHIP_BALANCE_THRESHOLD
    }
    throw error
  }

  if (!data) return DEFAULT_MEMBERSHIP_BALANCE_THRESHOLD
  return normalizeMembershipBalanceThreshold(data.membership_balance_threshold)
}

export async function saveMembershipBalanceThreshold(supabase, value) {
  const membershipBalanceThreshold = normalizeMembershipBalanceThreshold(value)
  const { data, error } = await supabase
    .from('admin_settings')
    .upsert({
      id: 1,
      membership_balance_threshold: membershipBalanceThreshold,
      updated_at: new Date().toISOString(),
    })
    .select('membership_balance_threshold')
    .single()

  if (error) throw error
  return normalizeMembershipBalanceThreshold(data.membership_balance_threshold)
}

export async function getFirstDepositBonusPercent(supabase, { allowDefaultOnMissingTable = true } = {}) {
  const { data, error } = await supabase
    .from('admin_settings')
    .select('first_deposit_bonus_percent')
    .eq('id', 1)
    .maybeSingle()

  if (error) {
    if (allowDefaultOnMissingTable && isMissingSettingsTable(error)) {
      return DEFAULT_FIRST_DEPOSIT_BONUS_PERCENT
    }
    throw error
  }

  if (!data) return DEFAULT_FIRST_DEPOSIT_BONUS_PERCENT
  return normalizeBonusPercent(data.first_deposit_bonus_percent)
}

export async function saveFirstDepositBonusPercent(supabase, value) {
  const firstDepositBonusPercent = normalizeBonusPercent(value)
  const { data, error } = await supabase
    .from('admin_settings')
    .upsert({
      id: 1,
      first_deposit_bonus_percent: firstDepositBonusPercent,
      updated_at: new Date().toISOString(),
    })
    .select('first_deposit_bonus_percent')
    .single()

  if (error) throw error
  return normalizeBonusPercent(data.first_deposit_bonus_percent)
}

export async function getPlatformLinks(supabase, { allowDefaultOnMissingTable = true } = {}) {
  const { data, error } = await supabase
    .from('admin_settings')
    .select('telegram_group_url, whatsapp_group_url, customer_support_url')
    .eq('id', 1)
    .maybeSingle()

  if (error) {
    if (allowDefaultOnMissingTable && isMissingSettingsTable(error)) {
      return {
        telegramGroupUrl: DEFAULT_TELEGRAM_GROUP_URL,
        whatsappGroupUrl: DEFAULT_WHATSAPP_GROUP_URL,
        customerSupportUrl: DEFAULT_CUSTOMER_SUPPORT_URL,
      }
    }
    throw error
  }

  return {
    telegramGroupUrl: normalizeExternalUrl(data?.telegram_group_url, 'Telegram group URL', DEFAULT_TELEGRAM_GROUP_URL),
    whatsappGroupUrl: normalizeExternalUrl(data?.whatsapp_group_url, 'WhatsApp group URL', DEFAULT_WHATSAPP_GROUP_URL),
    customerSupportUrl: normalizeExternalUrl(data?.customer_support_url, 'Customer support URL', DEFAULT_CUSTOMER_SUPPORT_URL),
  }
}

export async function savePlatformLinks(supabase, links = {}) {
  const platformLinks = {
    telegramGroupUrl: normalizeExternalUrl(links.telegramGroupUrl, 'Telegram group URL', DEFAULT_TELEGRAM_GROUP_URL),
    whatsappGroupUrl: normalizeExternalUrl(links.whatsappGroupUrl, 'WhatsApp group URL', DEFAULT_WHATSAPP_GROUP_URL),
    customerSupportUrl: normalizeExternalUrl(links.customerSupportUrl, 'Customer support URL', DEFAULT_CUSTOMER_SUPPORT_URL),
  }
  const { data, error } = await supabase
    .from('admin_settings')
    .upsert({
      id: 1,
      telegram_group_url: platformLinks.telegramGroupUrl,
      whatsapp_group_url: platformLinks.whatsappGroupUrl,
      customer_support_url: platformLinks.customerSupportUrl,
      updated_at: new Date().toISOString(),
    })
    .select('telegram_group_url, whatsapp_group_url, customer_support_url')
    .single()

  if (error) throw error
  return {
    telegramGroupUrl: normalizeExternalUrl(data.telegram_group_url, 'Telegram group URL', DEFAULT_TELEGRAM_GROUP_URL),
    whatsappGroupUrl: normalizeExternalUrl(data.whatsapp_group_url, 'WhatsApp group URL', DEFAULT_WHATSAPP_GROUP_URL),
    customerSupportUrl: normalizeExternalUrl(data.customer_support_url, 'Customer support URL', DEFAULT_CUSTOMER_SUPPORT_URL),
  }
}

export async function getWithdrawalSettings(supabase, { allowDefaultOnMissingTable = true } = {}) {
  const { data, error } = await supabase
    .from('admin_settings')
    .select('min_withdrawal_amount, max_withdrawal_amount, daily_withdrawal_limit, withdrawal_limit_exempt_usernames, withdrawal_fee_percent, withdrawals_enabled, withdrawal_disabled_message')
    .eq('id', 1)
    .maybeSingle()

  if (error) {
    if (allowDefaultOnMissingTable && isMissingSettingsTable(error)) {
      return {
        minWithdrawalAmount: DEFAULT_MIN_WITHDRAWAL_AMOUNT,
        maxWithdrawalAmount: WITHDRAWAL_HARD_LIMIT_AMOUNT,
        annualWithdrawalLimit: DEFAULT_ANNUAL_WITHDRAWAL_LIMIT,
        dailyWithdrawalLimit: DEFAULT_DAILY_WITHDRAWAL_LIMIT,
        withdrawalLimitExemptUsernames: [],
        withdrawalFeePercent: DEFAULT_WITHDRAWAL_FEE_PERCENT,
        withdrawalsEnabled: DEFAULT_WITHDRAWALS_ENABLED,
        withdrawalDisabledMessage: DEFAULT_WITHDRAWAL_DISABLED_MESSAGE,
      }
    }
    throw error
  }

  if (!data) {
    return {
      minWithdrawalAmount: DEFAULT_MIN_WITHDRAWAL_AMOUNT,
      maxWithdrawalAmount: WITHDRAWAL_HARD_LIMIT_AMOUNT,
      annualWithdrawalLimit: DEFAULT_ANNUAL_WITHDRAWAL_LIMIT,
      dailyWithdrawalLimit: DEFAULT_DAILY_WITHDRAWAL_LIMIT,
      withdrawalLimitExemptUsernames: [],
      withdrawalFeePercent: DEFAULT_WITHDRAWAL_FEE_PERCENT,
      withdrawalsEnabled: DEFAULT_WITHDRAWALS_ENABLED,
      withdrawalDisabledMessage: DEFAULT_WITHDRAWAL_DISABLED_MESSAGE,
    }
  }

  const minWithdrawalAmount = normalizeWithdrawalAmount(
    data.min_withdrawal_amount ?? DEFAULT_MIN_WITHDRAWAL_AMOUNT,
    'Minimum withdrawal amount'
  )
  const annualWithdrawalLimit = normalizeWithdrawalAmount(
    data.max_withdrawal_amount ?? DEFAULT_MAX_WITHDRAWAL_AMOUNT,
    'Annual withdrawal limit'
  )
  const dailyWithdrawalLimit = normalizeWithdrawalAmount(
    data.daily_withdrawal_limit ?? DEFAULT_DAILY_WITHDRAWAL_LIMIT,
    'Daily withdrawal limit'
  )

  if (annualWithdrawalLimit < minWithdrawalAmount || dailyWithdrawalLimit < minWithdrawalAmount) {
    const error = new Error('Withdrawal limits cannot be lower than the minimum withdrawal amount')
    error.statusCode = 400
    throw error
  }

  return {
    minWithdrawalAmount,
    maxWithdrawalAmount: WITHDRAWAL_HARD_LIMIT_AMOUNT,
    annualWithdrawalLimit,
    dailyWithdrawalLimit,
    withdrawalLimitExemptUsernames: normalizeExemptUsernames(data.withdrawal_limit_exempt_usernames),
    withdrawalFeePercent: normalizeWithdrawalFeePercent(
      data.withdrawal_fee_percent ?? DEFAULT_WITHDRAWAL_FEE_PERCENT
    ),
    withdrawalsEnabled: data.withdrawals_enabled ?? DEFAULT_WITHDRAWALS_ENABLED,
    withdrawalDisabledMessage: normalizeWithdrawalDisabledMessage(data.withdrawal_disabled_message),
  }
}

export async function saveWithdrawalSettings(supabase, settings = {}) {
  const nextSettings = {
    minWithdrawalAmount: settings.minWithdrawalAmount ?? DEFAULT_MIN_WITHDRAWAL_AMOUNT,
    annualWithdrawalLimit: settings.annualWithdrawalLimit ?? settings.maxWithdrawalAmount ?? DEFAULT_ANNUAL_WITHDRAWAL_LIMIT,
    dailyWithdrawalLimit: settings.dailyWithdrawalLimit ?? DEFAULT_DAILY_WITHDRAWAL_LIMIT,
    withdrawalLimitExemptUsernames: settings.withdrawalLimitExemptUsernames ?? [],
    withdrawalFeePercent: settings.withdrawalFeePercent ?? DEFAULT_WITHDRAWAL_FEE_PERCENT,
    withdrawalsEnabled: settings.withdrawalsEnabled ?? DEFAULT_WITHDRAWALS_ENABLED,
    withdrawalDisabledMessage: settings.withdrawalDisabledMessage ?? DEFAULT_WITHDRAWAL_DISABLED_MESSAGE,
  }

  const minWithdrawalAmount = normalizeWithdrawalAmount(
    nextSettings.minWithdrawalAmount,
    'Minimum withdrawal amount'
  )
  const annualWithdrawalLimit = normalizeWithdrawalAmount(
    nextSettings.annualWithdrawalLimit,
    'Annual withdrawal limit'
  )
  const dailyWithdrawalLimit = normalizeWithdrawalAmount(nextSettings.dailyWithdrawalLimit, 'Daily withdrawal limit')

  if (annualWithdrawalLimit < minWithdrawalAmount || dailyWithdrawalLimit < minWithdrawalAmount) {
    const error = new Error('Withdrawal limits cannot be lower than the minimum withdrawal amount')
    error.statusCode = 400
    throw error
  }

  if (minWithdrawalAmount > WITHDRAWAL_HARD_LIMIT_AMOUNT) {
    const error = new Error(`Minimum withdrawal amount cannot exceed the ${WITHDRAWAL_HARD_LIMIT_AMOUNT} FCFA hard limit`)
    error.statusCode = 400
    throw error
  }

  const withdrawalFeePercent = normalizeWithdrawalFeePercent(nextSettings.withdrawalFeePercent)
  const withdrawalsEnabled = Boolean(nextSettings.withdrawalsEnabled)
  const withdrawalDisabledMessage = normalizeWithdrawalDisabledMessage(nextSettings.withdrawalDisabledMessage)

  const { data, error } = await supabase
    .from('admin_settings')
    .upsert({
      id: 1,
      min_withdrawal_amount: minWithdrawalAmount,
      max_withdrawal_amount: annualWithdrawalLimit,
      daily_withdrawal_limit: dailyWithdrawalLimit,
      withdrawal_limit_exempt_usernames: normalizeExemptUsernames(nextSettings.withdrawalLimitExemptUsernames),
      withdrawal_fee_percent: withdrawalFeePercent,
      withdrawals_enabled: withdrawalsEnabled,
      withdrawal_disabled_message: withdrawalDisabledMessage,
      updated_at: new Date().toISOString(),
    })
    .select('min_withdrawal_amount, max_withdrawal_amount, daily_withdrawal_limit, withdrawal_limit_exempt_usernames, withdrawal_fee_percent, withdrawals_enabled, withdrawal_disabled_message')
    .single()

  if (error) throw error

  return {
    minWithdrawalAmount: normalizeWithdrawalAmount(data.min_withdrawal_amount, 'Minimum withdrawal amount'),
    maxWithdrawalAmount: WITHDRAWAL_HARD_LIMIT_AMOUNT,
    annualWithdrawalLimit: normalizeWithdrawalAmount(data.max_withdrawal_amount, 'Annual withdrawal limit'),
    dailyWithdrawalLimit: normalizeWithdrawalAmount(data.daily_withdrawal_limit, 'Daily withdrawal limit'),
    withdrawalLimitExemptUsernames: normalizeExemptUsernames(data.withdrawal_limit_exempt_usernames),
    withdrawalFeePercent: normalizeWithdrawalFeePercent(data.withdrawal_fee_percent),
    withdrawalsEnabled: data.withdrawals_enabled ?? DEFAULT_WITHDRAWALS_ENABLED,
    withdrawalDisabledMessage: normalizeWithdrawalDisabledMessage(data.withdrawal_disabled_message),
  }
}
