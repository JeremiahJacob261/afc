...
export async function getAdminDashboardData() {
  ...
  const [
    ...
    { count: betsToday, error: betsTodayError },
    { data: reading, error: readingError },
    { data: notifications, error: notificationsError },
    { data: latestUsers, error: latestUsersError },
    { data: latestMatches, error: latestMatchesError },
    { data: placed, error: placedError },
  ] = await Promise.all([
    supabase.from('users').select('*', { count: 'exact', head: true }),
    supabase.from('users').select('*', { count: 'exact', head: true }).eq('firstd', true),
    supabase
      .from('placed')
    ...
    supabase
      .from('users')
      .select('username,uid,created_at,balance,totald,firstd,countrycode,newrefer')
      .order('id', { ascending: false })
      .limit(6),
    supabase
      .from('bets')
