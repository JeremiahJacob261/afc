...
// The platform ledger is USDT. Payment methods can use another currency, so
// keep their code separate from the ledger label shown to users.
export function isUsdtPaymentCode(value) {
  ...
  return code === 'usdt'
    ...
    || code === 'usdt trc20'
    || code === 'usdt bep20'
    || tokens.includes('usdt')
    || tokens.includes('tether')
}

export function displayPaymentCurrency(value) {
  return isUsdtPaymentCode(value) ? 'USDT' : displayPaymentCode(value || 'USDT')
}
...
// A payment-method rate is expressed as payment-currency units per 1 USDT.
export function toLedgerAmount(paymentAmount, rate) {
...
export function fromLedgerAmount(ledgerAmount, rate) {
...
// Compatibility aliases for older callers. The ledger currency is USDT.
export const isFcfaPaymentCode = isUsdtPaymentCode
export const toFcfaLedgerAmount = toLedgerAmount
export const fromFcfaLedgerAmount = fromLedgerAmount
